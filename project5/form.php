<form novalidate class="flexform" action="" method="POST">
  ...
  <div class="input-block margin-bottom">
    ...
    <div class="radiobtn-block">
      Мужской <input name="sex" value="m" type="radio" <?= $values['sex'] == "m" ? 'checked="checked"' : '' ?> >
      Женский <input name="sex" value="w" type="radio" <?= $values['sex'] == "w" ? 'checked="checked"' : '' ?> >
    </div>
    <?php if ($errors['sex']) {print '<p class="err-msg">'.$messages['sex'].'</p>'; } ?>
  </div>

  <div class="input-block margin-bottom">
    <label for="limb">Количество конечностей:</label>
    <div class="radiobtn-block">
      1 <input name="limb" value="1" type="radio" <?= 1 == $values['limb'] ? 'checked="checked"' : '' ?> >
      2 <input name="limb" value="2" type="radio" <?= 2 == $values['limb'] ? 'checked="checked"' : '' ?> >
      3 <input name="limb" value="3" type="radio" <?= 3 == $values['limb'] ? 'checked="checked"' : '' ?> >
      4 <input name="limb" value="4" type="radio" <?= 4 == $values['limb'] ? 'checked="checked"' : '' ?> >
    </div>
    <?php if ($errors['limb']) {print '<p class="err-msg">'.$messages['limb'].'</p>'; } ?>
  </div>
  ...
  <?php
    ...
    if(empty($_SESSION['login'])) {
      $buttons = '<a class="linked_btn" href="login.php">Уже регистрировались?</a>';
    }
    else {
      $buttons = '<a class="linked_btn" href="logout.php">Выйти</a>';
    }
    print($buttons);
